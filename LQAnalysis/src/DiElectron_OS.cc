// $Id: DiElectron_OS.cc 1 2013-11-26 10:23:10Z jalmond $
/***************************************************************************
 * @Project: LQDiElectron_OS Frame - ROOT-based analysis framework for Korea SNU
 * @Package: LQCycles
 *
...
ClassImp (DiElectron_OS);
  ...
  *
  */
DiElectron_OS::DiElectron_OS() :  AnalyzerCore(), out_muons(0)  {
  
  
  // To have the correct name in the log:                                                                                                                            
  SetLogName("DiElectron_OS");
  
  Message("In DiElectron_OS constructor", INFO);
  //
...
void DiElectron_OS::InitialiseAnalysis() throw( LQError ) {
...
void DiElectron_OS::ExecuteEvents()throw( LQError ){
...
//  std::vector<snu::KElectron> electrons =  GetElectrons(false,false,"ELECTRON_NOCUT");
  std::vector<snu::KElectron> electrons =  GetElectrons(false, false, BaseSelection::ELECTRON_POG_TIGHT);   
...
//     reco_weight = ElectronRecoScaleFactor(electrons);
  ...
  if(electrons.size() == 2){
    ...
    if(electrons.at(0).Charge()*electrons.at(1).Charge() == 1) return; //require OS 
    FillCutFlow("OppositeSign", ev_weight);
...
void DiElectron_OS::EndCycle()throw( LQError ){
...
void DiElectron_OS::BeginCycle() throw( LQError ){
...
DiElectron_OS::~DiElectron_OS() {
  
  Message("In DiElectron_OS Destructor" , INFO);
  
}

void DiElectron_OS::FillCutFlow(TString cut, float weight){
  ...
  if(GetHist("cutflow")) {
    ...
    GetHist("cutflow")->GetXaxis()->SetBinLabel(6,"OppositeSign");
...
void DiElectron_OS::BeginEvent( )throw( LQError ){
...
void DiElectron_OS::MakeHistograms(){
  ...
  /**
   *  Remove//Overide this DiElectron_OSCore::MakeHistograms() to make new hists for your analysis
   **/
  
}


void DiElectron_OS::ClearOutputVectors() throw(LQError) {
