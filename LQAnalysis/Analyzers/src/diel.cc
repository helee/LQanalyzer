...
//      ConfigureFakeHists("/data1/LQAnalyzer_rootfiles_for_analysis/CATAnalysis2016/Fake/DiLep/", fake_hists);
  ...
  std::vector<snu::KElectron> electrons_tight = GetElectrons(false, false, "ELECTRON_HN_TIGHTv4");
          ...
          if((muons_veto.size() == 0) && (electrons_veto.size() == 2)){
            ...
            if((X.M() > 10.) && (fabs(X.M()-91.1876) > 10.)){
              ...
              if(njet > 1){
                ...
                if(electrons_tight.size() == 0) FillHist("Presel_Nevent_unweight", 0.5, 1., 0., 3., 3);
                if(electrons_tight.size() == 1) FillHist("Presel_Nevent_unweight", 1.5, 1., 0., 3., 3);
                if(electrons_tight.size() == 2) FillHist("Presel_Nevent_unweight", 2.5, 1., 0., 3., 3);
